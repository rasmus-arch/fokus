...
async function buildPdfImageCell(imageUrl) {
    ...
    if (rawUrl.startsWith('http')) {
        ...
        if (base64) { const mime = rawUrl.toLowerCase().includes('.png') ? 'image/png' : 'image/jpeg'; return { image: `data:${mime};base64,${base64}`, width: 45, height: 45, alignment: 'center', margin: [0, 5, 0, 5] }; }
        return emptyCell;
    }
    ...
    if (fs.existsSync(fullImgPath)) {
        ...
        if (ext === '.jpg' || ext === '.jpeg' || ext === '.png') {
            try { const mime = ext === '.png' ? 'image/png' : 'image/jpeg'; return { image: `data:${mime};base64,${fs.readFileSync(fullImgPath).toString('base64')}`, width: 45, height: 45, alignment: 'center', margin: [0, 5, 0, 5] }; } catch (e) {}
        }
    }
    return emptyCell;
}
