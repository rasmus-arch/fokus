...
async function buildPdfImageCell(imageUrl, size = 45) {
    ...
    if (fs.existsSync(fullImgPath)) {
        ...
        if (ext === '.jpg' || ext === '.jpeg' || ext === '.png') {
            try {
                if (fs.statSync(fullImgPath).size > MAX_EMBED_IMAGE_BYTES) return emptyCell;
                const mime = ext === '.png' ? 'image/png' : 'image/jpeg'; return { image: `data:${mime};base64,${fs.readFileSync(fullImgPath).toString('base64')}`, width: size, height: size, alignment: 'center', margin: [0, 5, 0, 5] };
            } catch (e) {}
        }
    }
    return emptyCell;
