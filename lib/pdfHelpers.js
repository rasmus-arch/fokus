...
// Övre gräns för hur stora bildfiler som bäddas in i en PDF - utan detta kan en enda
// obeskuren telefonkamerabild (flera MB) driva upp minnesanvändningen tillräckligt för att
// krascha Node-processen på delad hosting, vilket visar sig som ett obegripligt 503-fel.
const MAX_EMBED_IMAGE_BYTES = 8 * 1024 * 1024; // 8 MB
...
// om det inte går. PDFKit (som pdfmake bygger på) stödjer bara JPEG/PNG - andra format (t.ex.
// AVIF/HEIC från mobilkameror) hoppas över istället för att bäddas in trasiga.
async function readEmbeddableImage(fullImgPath) {
    ...
    try {
        if (fs.statSync(fullImgPath).size > MAX_EMBED_IMAGE_BYTES) return null;
        return { base64: fs.readFileSync(fullImgPath).toString('base64'), mime: ext === '.png' ? 'image/png' : 'image/jpeg' };
    } catch (e) { return null; }
}
